...
for i = 1:maxorder, % go up to 6*4 sources, plus the first one at -x_s (so, 25)
    wall = 2*i*L;
    sources = [-wall-x_s, wall-x_s, -wall+x_s, wall+x_s];
...
sfactor = (1+dist/k).^(-0.2);

for nr = 1:length(orders),
    impReflection = [zeros(1,delay(nr)), sfactor(nr) * wallReflection(:,orders(nr)+1)', zeros(1,m*implen-delay(nr)-length(dirac))]';
